...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--kb-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--kb-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[swipe=end]:animate-out ui-opened:animate-in ui-opened:slide-in-from-top-full ui-closed:animate-out ui-closed:fade-out-80 ui-closed:slide-out-to-right-full ui-opened:sm:slide-in-from-top-full",
  {
    variants: {
      variant: {
...
export function ToastRoot(
  props: ComponentProps<typeof Toast> & VariantProps<typeof toastVariants>,
) {
...
export function ToastContent(props: JSX.HTMLAttributes<HTMLDivElement>) {
  const [_, rest] = splitProps(props, ["class"]);
  return <div class={cn("flex w-full flex-col gap-2")} {...rest} />;
}
...
export function ToastTitle(props: ComponentProps<typeof Toast.Title>) {
  const [_, rest] = splitProps(props, ["class"]);
  return (
    <Toast.Title class={cn("text-sm font-semibold", props.class)} {...rest} />
  );
}
...
export function ToastProgressTrack(
  ...
  return (
    <Toast.ProgressTrack
      class={cn("h-2 w-full rounded bg-primary-foreground", props.class)}
      {...rest}
    />
  );
}
...
export function ToastProgressFill(
  ...
  return (
    <Toast.ProgressFill
      class={cn(
        "h-full w-[var(--kb-toast-progress-fill-width)] rounded bg-primary transition-all",
        props.class,
      )}
      {...rest}
    />
...
function custom(
  ele: () => JSX.Element,
  rootProps?: Omit<ComponentProps<typeof ToastRoot>, "toastId">,
) {
  return toaster.show((props) => (
    <ToastRoot toastId={props.toastId} {...rootProps}>
      {ele as any}
    </ToastRoot>
  ));
}
